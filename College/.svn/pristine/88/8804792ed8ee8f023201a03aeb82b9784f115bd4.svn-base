$(document).ready(function() {
	...
	if (ajax_version) {
		...
		setTimeout(function() {
					...
					$('ul.nav-list li:has(a[href="' + type + '"])')
							.addClass('active').closest('.submenu')
							.addClass('current').find('ul').css('display',
									'block');
				}, 2000);
	...
	$('.nav-input-search').typeahead({
				source : [{
							...
							id : 'delighted-layouts',
							name : 'Delighted Layouts'
						}, {
							id : 'grids',
							name : 'Grids'
						}, {
...
function init() {
	...
	$('.fa-hover').click(function(e) {
		...
		$('.icon-show').html('<i class="' + valued
				+ ' fa-5x "></i>&nbsp;&nbsp;<i class="' + valued
				+ ' fa-4x "></i>&nbsp;&nbsp;<i class="' + valued
				+ ' fa-3x "></i>&nbsp;&nbsp;<i class="' + valued
				+ ' fa-2x "></i>&nbsp;&nbsp;<i class="' + valued
				+ ' "></i>&nbsp;&nbsp;');
	...
	$('.panel-settings').click(function(e) {
		...
		// $('.modal-title').html(valued);
		// var
		// vj=$(this).parent().parent().parent().parent().parent().getElementById();
	...
	$('.panel-refresh').on('click', function(e) {
				...
				var $target = $(this).parent().parent().parent()
						.next('.panel-body');
				$target
						.mask('<i class="fa fa-refresh fa-spin"></i> Loading...');
...
function loadPage(url, container) {
	...
	$.ajax({
		...
		success : function() {
			container
					.mask('<h1><i class="fa  fa-refresh fa-spin"></i> Loading...</h1>');
...
//						alert(url);
		...
		error : function(xhr, ajaxOptions, thrownError) {
			// container.html('<h4 style="margin-top:10px; display:block;
			// text-align:left"><i class="fa fa-warning
			// txt-color-orangeDark"></i> Error 404! Page not found.</h4>');
			container.load('404.html');
			setTimeout(function() {
						loadPage('dashboard', container)
						$('ul.nav-list li ').removeClass('active');
						$('ul.nav-list li:has(a[href="dashboard"])')
								.addClass('active').closest('.submenu')
								.addClass('current').find('ul').css('display',
										'block');
					}, 3000);
...
function sortablePortlets() {
	$(".grid").sortable({
				...
				stop : function(evt, ui) {
					// console.log($(".grid").sortable('toArray', { attribute:
					// 'data-name' }));
				}
			});
	...
	if (localStorage.getItem("sorted") !== null) {
		var arrValuesForOrder = localStorage.getItem('sorted').substring(6)
				.split("&div[]=");
		...
		for (var i = arrValuesForOrder[arrValuesForOrder.length - 1]; i >= 0; i--) {
			// index is zero-based to you have to remove one from the values in
			// your array
